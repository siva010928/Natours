...
const multerFilter = (req, file, cb) => {
  if (file.mimetype.startsWith('image')) {
    ...
    cb(new AppError('image only accepted', 402), false);
  }
};
...
exports.getCheaptours = (req, res, next) => {
  ...
  req.query.fields = 'price,summary,name,ratingsAverage';
  next();
};
...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  if (!lat || !lng) {
    return next(new AppError('provide centre in format of lat,lng'), 401);
  }
